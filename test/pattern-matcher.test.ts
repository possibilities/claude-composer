...
describe('PatternMatcher', () => {
  ...
  describe('Tree Pattern Matching', () => {
    ...
    it('should not match ~~tree~~ without cursor', () => {
      ...
      // Should not match if there's no cursor after the space
      const matches1 = matcher.processData('~~tree~~ some text')
      expect(matches1).toHaveLength(0)

      // Should not match if there's no space before cursor
      const matches2 = matcher.processData('~~tree~~\x1b[7m \x1b[0m')
      expect(matches2).toHaveLength(0)
    })
    ...
    it('should not match ~~tree~~ when user continues typing after space', () => {
      ...
      // Should not match if user continues typing after the space
      const matches = matcher.processData('~~tree~~ some additional text')
      expect(matches).toHaveLength(0)
    })
  })
