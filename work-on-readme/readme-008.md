...
Roots define trusted directories where certain operations are automatically allowed:

```yaml
roots:
  - ~/projects # Trust all subdirectories
  - /tmp/sandbox # Temporary trusted location
```

When working in a trusted root:

- File operations are streamlined
- Certain confirmations are bypassed based on ruleset
- Project-level patterns take precedence
